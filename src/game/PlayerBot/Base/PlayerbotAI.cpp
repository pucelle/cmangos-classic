...
bool PlayerbotAI::ItemStatComparison(const ItemPrototype* pProto, const ItemPrototype* pProto2)
{
    ...
    for (int i = 0; i < MAX_ITEM_PROTO_STATS; ++i) // item can only have 10 stats. We check each stat slot available for stat and type.
    {
        ...
        if (itemmod == ITEM_MOD_MANA || itemmod == ITEM_MOD_INTELLECT || itemmod == ITEM_MOD_SPIRIT ||
                itemmod2 == ITEM_MOD_MANA || itemmod2 == ITEM_MOD_INTELLECT || itemmod2 == ITEM_MOD_SPIRIT)
        {
...
void PlayerbotAI::SendOrders(Player& /*player*/)
{
    ...
    if (!m_combatOrder)
        ...
        out << "I HEAL but will ignore main tank";
    else if (m_combatOrder & ORDERS_PASSIVE)
        out << "I'm PASSIVE";

    if (m_combatOrder & ORDERS_DPS)
        out << (out.str().length() > 0 ? "," : "I") << " do DPS";

    if (m_combatOrder & ORDERS_HEAL)
        out << (out.str().length() > 0 ? "," : "I") << " HEAL";

    if (m_combatOrder & ORDERS_ASSIST)
        out << (out.str().length() > 0 ? "," : "I") << " ASSIST " << (m_targetAssist ? m_targetAssist->GetName() : "unknown");
    ...
    switch (packet.GetOpcode())
    {
        ...
        case SMSG_TRADE_STATUS:
        {
            ...
            else if (status == 1)
            {
                ...
                for (int8 quality = ITEM_QUALITY_LEGENDARY; quality >= ITEM_QUALITY_POOR; quality--)
                {
                    uint8 count = 0;
                ...
                // how many bat slots the bot has.
            ...
            if (guid == m_lootPrev)
            {
                ...
                if (c && c->GetCreatureInfo()->SkinningLootId && c->GetLootStatus() != CREATURE_LOOT_STATUS_LOOTED)
                {
                    ...
                    if (m_bot->HasSkill(reqSkill) && ((reqSkill != SKILL_SKINNING) ||
                                                      (HasCollectFlag(COLLECT_FLAG_SKIN) && reqSkill == SKILL_SKINNING && !m_ignoreSkinning)))
                    {
    ...
    for (uint8 slot = INVENTORY_SLOT_ITEM_START; slot < INVENTORY_SLOT_ITEM_END; slot++)
    {
        ...
        if (pItem)
        {
            ...
            if (pItemProto->Class == ITEM_CLASS_CONSUMABLE &&
                (pItemProto->SubClass == ITEM_SUBCLASS_FOOD || pItemProto->SubClass == ITEM_SUBCLASS_CONSUMABLE))
                // if is FOOD
...
Item* PlayerbotAI::FindBandage() const
{
    ...
    for (uint8 slot = INVENTORY_SLOT_ITEM_START; slot < INVENTORY_SLOT_ITEM_END; slot++)
    {
        ...
        if (pItem)
        {
            ...
            if (pItemProto->Class == ITEM_CLASS_CONSUMABLE && pItemProto->Spells[0].SpellCategory == 150)
                return pItem;
        }
    }
    // list out items in other removable backpacks
...
static const uint32 uPriorizedDamageOilDisplayIds[4] =
{
    MINOR_WIZARD_OIL,
    LESSER_WIZARD_OIL,
...
static const uint32 uPriorizedHealingOilDisplayIds[3] =
{
    MINOR_MANA_OIL,
    LESSER_MANA_OIL,
    BRILLIANT_MANA_OIL,
...
/**
 * FindOilFor()
 * Returns damage/healing oil item eligible to enchant a bot weapon.
 */
Item* PlayerbotAI::FindOilFor(bool forHealing) const
{
    Item* oil;
    if (forHealing)
    {
        for (uint8 i = 0; i < countof(uPriorizedHealingOilDisplayIds); ++i)
        {
            oil = FindConsumable(uPriorizedHealingOilDisplayIds[i]);
            if (oil)
                return oil;
        }
    }
    ...
    {
        for (uint8 i = 0; i < countof(uPriorizedDamageOilDisplayIds); ++i)
        {
            oil = FindConsumable(uPriorizedDamageOilDisplayIds[i]);
            if (oil)
                return oil;
        }
    }
...
void PlayerbotAI::DoCombatMovement()
{
             ...
             && GetClassAI()->GetWaitUntil() == 0)  // Not waiting
    {
        ...
        if (!CanReachWithSpellAttack(m_targetCombat) && !IsOnlyHealer())
        {
            m_bot->GetMotionMaster()->Clear(false);
            m_bot->GetMotionMaster()->MoveChase(m_targetCombat, 20.0f, m_bot->GetOrientation());
        }

        // If target is not in sight, will also move closer to it.
        else if (!m_bot->IsWithinLOSInMap(m_targetCombat) && !IsOnlyHealer())
        {
            m_bot->GetMotionMaster()->Clear(false);
            m_bot->GetMotionMaster()->MoveChase(m_targetCombat, 10.0f, m_bot->GetOrientation());
        }
        else
            MovementClear();
    }
...
void PlayerbotAI::DoLoot()
{
    ...
    if (c)
    {
        ...
        // not a lootable creature, or can skinning but has no tool, clear it
        if (!c->HasFlag(UNIT_DYNAMIC_FLAGS, UNIT_DYNFLAG_LOOTABLE) ||
            (c->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_SKINNABLE) && m_bot->HasSkill(skillId) && !m_ignoreSkinning)
        )
        {
            m_lootCurrent = ObjectGuid();
            ...
            if (c->HasFlag(UNIT_DYNAMIC_FLAGS, UNIT_DYNFLAG_LOOTABLE) && !c->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_SKINNABLE))
            {
                ...
                *packet << m_lootCurrent;
                ...
                if (m_bot->HasSkill(skillId) && ((skillId != SKILL_SKINNING) ||
                                                 (HasCollectFlag(COLLECT_FLAG_SKIN) && skillId == SKILL_SKINNING && !m_ignoreSkinning)))
                {
        ...
        if ((m_bot->HasSkill(skillId) && skillId != SKILL_NONE) || (skillId == SKILL_NONE && go))
        {
            ...
            switch (skillId)
            {
                ...
                case SKILL_HERBALISM:
                    ...
                    break;
                case SKILL_SKINNING:
                    if (c && HasCollectFlag(COLLECT_FLAG_SKIN) && HasTool(TC_SKINNING_KNIFE) && CastSpell(SKINNING, *c) == SPELL_CAST_OK)
                        return;
                    else
                        skillFailed = true;
                    break;
...
void PlayerbotAI::SetCombatOrderByStr(std::string str, Unit* target)
{
    ...
    else if (str == "dps")          co = ORDERS_DPS;
    else if (str == "reset")        co = ORDERS_RESET;
    else return;
...
void PlayerbotAI::SetCombatOrder(CombatOrderType co, Unit* target)
{
    ...
    {
        m_combatOrder = (CombatOrderType)(((uint32) m_combatOrder & (uint32) (ORDERS_SECONDARY | ORDERS_HEAL | ORDERS_DPS)) | (uint32) co);
        if (target)
...
void PlayerbotAI::UpdateAI(const uint32 /*p_time*/)
{
    ...
    // default updates occur every 1 second.
    SetIgnoreUpdateTime(1);
    ...
    if (!m_bot->IsAlive())
    {
        ...
        if (m_botState == BOTSTATE_DEADRELEASED)
        {
            ...
            time_t resurrect_time = corpse->GetGhostTime() + m_bot->GetCorpseReclaimDelay(corpse->GetType() == CORPSE_RESURRECTABLE_PVP / 2.0f);
    ...
    // so no need to make sure any actions that cast a spell set a proper m_ignoreAIUpdatesUntilTime!
    Spell* const pSpell = GetCurrentSpell();
    if (pSpell && (pSpell->IsChannelActive() || pSpell->IsAutoRepeat()))
    {
        // DEBUG_LOG("spell (%s) is being casting",pSpell->m_spellInfo->SpellName[0]);
        return;
    }
...
bool PlayerbotAI::In_Range(Unit* Target, uint32 spellId)
{
    ...
    float maxRange = TempRange->maxRange;

    // The max range may be enhanced from player's talent.
    SpellRanges::iterator it = m_spellRangeMap.find(spellId);
    if (it != m_spellRangeMap.end())
        maxRange = std::max(maxRange, it->second);

    //Unit is out of range of this spell
    if (!m_bot->IsInRange(Target, TempRange->minRange, maxRange, true))
        return false;
...
SpellCastResult PlayerbotAI::CastSpell(uint32 spellId)
{
    ...
    else
    {
        ...
        if (IsAutoRepeatRangedSpell(pSpellInfo))
            castResult = m_bot->CastSpell(pTarget, pSpellInfo, TRIGGERED_OLD_TRIGGERED); // cast triggered spell
        else
            castResult = m_bot->CastSpell(pTarget, pSpellInfo, TRIGGERED_NONE);          // uni-cast spell

        if (castResult == SPELL_CAST_OK) {
            // Some casting times are negative so set ignore update time to 1 sec to avoid stucking the bot AI
            SetIgnoreUpdateTime(std::max(CastTime, 1.0f));
        }
...
SpellCastResult PlayerbotAI::Buff(uint32 spellId, Unit* target, void (*beforeCast)(Player*))
{
    ...
    if (target->HasAura(spellProto->Id))
    ...
    for (uint8 i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        ...
        for (Unit::AuraList::const_iterator it = auras.begin(); it != auras.end(); ++it)
            if ((*it)->GetModifier()->m_miscvalue == spellProto->EffectMiscValue[i]
                ...
                && (*it)->GetModifier()->periodictime > 0)  // Ignores passive buff.
            {
...
bool PlayerbotAI::HasTool(uint32 TC)
{
    ...
    switch (TC)
    {
        ...
        case TC_SKINNING_KNIFE:            //  = 166
            ...
            if (m_bot->HasItemCount(7005, 1) || m_bot->HasItemCount(12709, 1))
                return true;
            else {
                out << "|cffff0000I do not have a SKINNING KNIFE!";
                m_ignoreSkinning = true;
            }
            break;

        case TC_COPPER_ROD:                //  = 6,
            if (m_bot->HasItemCount(6218, 1) || m_bot->HasItemCount(6339, 1) || m_bot->HasItemCount(11130, 1)
                || m_bot->HasItemCount(11145, 1) || m_bot->HasItemCount(16207, 1))
                return true;
            else
        ...
        case TC_SILVER_ROD:                //  = 7,
            if (m_bot->HasItemCount(6339, 1) || m_bot->HasItemCount(11130, 1)
                || m_bot->HasItemCount(11145, 1) || m_bot->HasItemCount(16207, 1))
                return true;
            else
        ...
        case TC_GOLDEN_ROD:                //  = 8,
            if (m_bot->HasItemCount(11130, 1) || m_bot->HasItemCount(11145, 1) || m_bot->HasItemCount(16207, 1))
                return true;
            else
        ...
        case TC_TRUESILVER_ROD:            //  = 9,
            if (m_bot->HasItemCount(11145, 1) || m_bot->HasItemCount(16207, 1))
                return true;
            else
...
void PlayerbotAI::findNearbyCorpse()
{
    ...
    for (UnitList::const_iterator i = corpseList.begin(); i != corpseList.end(); ++i)
    {
        ...
        if (corpse->HasFlag(UNIT_DYNAMIC_FLAGS, UNIT_DYNFLAG_LOOTABLE) ||
                (corpse->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_SKINNABLE) && m_bot->HasSkill(skillId)) && !m_ignoreSkinning)
        {
...
bool PlayerbotAI::TradeItem(const Item& item, int8 slot)
{
    ...
    else
    {
        if (!item.CanBeTraded())
        {
            if (pTrade->GetItem(TradeSlots(TRADE_SLOT_NONTRADED)) == nullptr)
            {
...
void PlayerbotAI::SellGarbage(bool bListNonTrash, bool bDetailTrashSold, bool bVerbose)
{
    ...
    if (bVerbose)
    {
        if (SoldQuantity == 0 && goods.str().size() == 0)
...
void PlayerbotAI::HandleCommand(const std::string& text, Player& fromPlayer)
{
    ...
    if (m_bot->GetTrader() && m_bot->GetTrader()->GetObjectGuid() == fromPlayer.GetObjectGuid())
    {
        ...
        if (itemIds.size() == 0)
        {
            // Can list items even in tradding.
            if (ExtractCommand("items", input, true))
                _HandleCommandItems(input, fromPlayer);
            else
    ...
    else if (ExtractCommand("do", input))
        _HandleCommandDo(input, fromPlayer);
    ...
    else if (ExtractCommand("loot", input, true)) // true -> "loot" OR "l"
        _HandleCommandLoot(input, fromPlayer);

    // Handle all collection related commands here
    else if (ExtractCommand("collect", input))
        _HandleCommandCollect(input, fromPlayer);

    else if (ExtractCommand("quest", input, true))
    ...
    else if (ExtractCommand("disenchant", input, true)) // true -> "disenchant" OR "d"
        _HandleCommandDisenchant(input, fromPlayer);
    ...
    else if (ExtractCommand("stats", input, true)) // true -> "stats" OR "s"
        _HandleCommandStats(input, fromPlayer);

    // Summon bot to the position of master.
    else if (ExtractCommand("summon", input, true)) // true -> "summon" OR "s"
        _HandleCommandSummon(input, fromPlayer);

    // List bot's items.
    else if (ExtractCommand("items", input, true)) // true -> "items" OR "i"
        _HandleCommandItems(input, fromPlayer);

    // Give items to another player.
    else if (ExtractCommand("give", input))
        _HandleCommandGive(input, fromPlayer);

    // Logout.
    else if (ExtractCommand("logout", input, true) || ExtractCommand("remove", input, true)) // true -> "remove" OR "r", "logout" or "l"
        _HandleCommandLogout(fromPlayer);
...
void PlayerbotAI::_HandleCommandDo(std::string& text, Player& fromPlayer)
{
    ...
    else
    {
        ...
        if (text.empty())
        {
            SendWhisper("|cffff0000Syntax error:|cffffffff do <botName> <reset | tank | heal | passive | dps | info><assist | protect [targetPlayer]>", fromPlayer);
            return;
        }
...
void PlayerbotAI::_HandleCommandTalent(std::string& text, Player& fromPlayer)
{
    ...
    else
    {
        ...
        if (gold > 0)
            out << "Cost to reset all Talents is " << gold << "G";
...
void PlayerbotAI::_HandleCommandDisenchant(std::string& text, Player& fromPlayer)
{
    uint32 spellId;
    ...
    if (itemList.empty())
    {
        SendWhisper("|cffff0000I can't disenchant that!", fromPlayer);
        return;
    }
...
void PlayerbotAI::_HandleCommandGet(std::string& text, Player& fromPlayer)
{
    ...
    result = WorldDatabase.PQuery("SELECT * FROM (SELECT gameobject.guid, id, position_x, position_y, position_z, map, "
        "(POW(position_x - %f, 2) + POW(position_y - %f, 2) + POW(position_z - %f, 2)) AS order_ FROM gameobject "
        "LEFT OUTER JOIN game_event_gameobject on gameobject.guid=game_event_gameobject.guid WHERE map = '%i' %s ORDER BY order_ ASC LIMIT 1) AS temp_ WHERE order_<625",
        m_bot->GetPositionX(), m_bot->GetPositionY(), m_bot->GetPositionZ(), m_bot->GetMapId(), eventFilter.str().c_str());

    if (!result || result->GetRowCount() == 0)
    {
        SendWhisper("No object found within 25 yards.", fromPlayer);
        return;
    }
    ...
    ObjectGuid oGUID = ObjectGuid(HIGHGUID_GAMEOBJECT, entry, guid);
    GameObject* go = m_bot->GetMap()->GetGameObject(oGUID);

    if (!go)
    {
        SendWhisper("Object is not exist.", fromPlayer);
        return;
    }

    if (!go->IsSpawned())
    {
        SendWhisper("Object is not spawnable so I will not take it.", fromPlayer);
        return;
    }

    if (!m_lootCurrent.IsEmpty()) {
        m_lootTargets.push_front(oGUID);
    }
...
void PlayerbotAI::_HandleCommandLoot(std::string& text, Player& fromPlayer)
{
...
void PlayerbotAI::_HandleCommandStats(std::string& text, Player& fromPlayer)
{
    ...
    // calculate how much money bot has
    uint32 copper = m_bot->GetMoney();
    out << "|h|cffffffff to repair & has " << "|r|cff00ff00";
    out << Cash(copper);
    ChatHandler ch(&fromPlayer);
    ch.SendSysMessage(out.str().c_str());
}
...
void PlayerbotAI::_HandleCommandHelp(std::string& text, Player& fromPlayer)
{
    ...
    if (bMainHelp || ExtractCommand("do", text))
    {
        ch.SendSysMessage(_HandleCommandHelpHelper("do", "Shows you my orders. Free will is overrated, right?").c_str());

        if (!bMainHelp)
        {
            SendWhisper(_HandleCommandHelpHelper("do <tank | heal | assist | protect | reset> [targetPlayer]", "Sets general orders I should follow. Assist and Protect require a target."), fromPlayer);
            SendWhisper(_HandleCommandHelpHelper("do delay <0-10>", "Activates a delay before I start fighting."), fromPlayer);
            SendWhisper(_HandleCommandHelpHelper("do resume", "Resume combat orders to what they were before logout."), fromPlayer);

            // Catches all valid subcommands, also placeholders for potential future sub-subcommands
            if (ExtractCommand("combat", text, true))
            {
                SendWhisper(_HandleCommandHelpHelper("do tank", "Order me to tank. Best used on paladins, warriors or druids."), fromPlayer);
                SendWhisper(_HandleCommandHelpHelper("do heal", "Order me to heal. Best used on shamans, priests, druids or paladins."), fromPlayer);
                SendWhisper(_HandleCommandHelpHelper("do assist", "Assist the linked target focusing our killing power.", HL_TARGET), fromPlayer);
                SendWhisper(_HandleCommandHelpHelper("do protect", "Protect the listed target, attempting to keep aggro away from the target.", HL_TARGET), fromPlayer);
                SendWhisper(_HandleCommandHelpHelper("do reset", "Resets my combat orders as though you'd never given me any at all."), fromPlayer);
...
// Teleport bot to the position of master.
void PlayerbotAI::_HandleCommandSummon(std::string& text, Player& fromPlayer)
{
    WorldLocation loc;
...
// List bot's items.
void PlayerbotAI::_HandleCommandItems(std::string& text, Player& fromPlayer)
{
    std::list<ItemQualities>* qualities = new std::list<ItemQualities>;

    // Show all items.
    if (text == "") {
        qualities->push_back(ITEM_QUALITY_LEGENDARY);
        qualities->push_back(ITEM_QUALITY_EPIC);
        qualities->push_back(ITEM_QUALITY_RARE);
        qualities->push_back(ITEM_QUALITY_UNCOMMON);
        qualities->push_back(ITEM_QUALITY_NORMAL);
        qualities->push_back(ITEM_QUALITY_POOR);
    }

    if (ExtractCommand("grey", text) || ExtractCommand("poor", text))
    {
        qualities->push_back(ITEM_QUALITY_POOR);
    }
    else if (ExtractCommand("white", text) || ExtractCommand("normal", text))
    {
        qualities->push_back(ITEM_QUALITY_NORMAL);
    }
    else if (ExtractCommand("green", text) || ExtractCommand("common", text))
    {
        qualities->push_back(ITEM_QUALITY_UNCOMMON);
    }
    else if (ExtractCommand("blue", text) || ExtractCommand("rare", text))
    {
        qualities->push_back(ITEM_QUALITY_RARE);
    }
    else if (ExtractCommand("purple", text) || ExtractCommand("epic", text))
    {
        qualities->push_back(ITEM_QUALITY_EPIC);
    }
    else if (ExtractCommand("orange", text) || ExtractCommand("legendary", text))
    {
        qualities->push_back(ITEM_QUALITY_LEGENDARY);
    }
    ...
    if (!qualities->empty())
    {
        for (std::list<ItemQualities>::iterator it = qualities->begin(); it != qualities->end(); ++it)
        {
            ItemQualities quality = *it;
            ...
            if (!out.str().empty()) {
                SendWhisper(out.str().c_str(), *GetMaster());
                out.str("");
            }
        }
    }

    else if (ExtractCommand("tradable", text, true))
    {
        // list out items in main backpack
        for (uint8 slot = INVENTORY_SLOT_ITEM_START; slot < INVENTORY_SLOT_ITEM_END; slot++)
        {
    ...
    else if (ExtractCommand("binded", text, true))
    {
        // list out items in main backpack
        for (uint8 slot = INVENTORY_SLOT_ITEM_START; slot < INVENTORY_SLOT_ITEM_END; slot++)
        {
    ...
    else if (ExtractCommand("equipped", text, true))
    {
        // list out items in main backpack
    ...
    if (count == 0)
        TellMaster("I have no items in this type.");
...
// _HandleCommandGive: Handle giving items to another player.
// give player_name [Item Link][Item Link]
void PlayerbotAI::_HandleCommandGive(std::string& text, Player& fromPlayer)
{
    if (text.empty())
    {
        TellMaster("give must be used with a player name and one or more item links (shift + click the item).");
        return;
    }

    // Find position of space.
    int nameEndPos = text.find(" ", 0);
    if (nameEndPos <= 0)
    {
        TellMaster("Can't find a player name");
        return;
    }

    std::string playerName = text.substr(0, nameEndPos);
    ObjectGuid playerGuid = sObjectMgr.GetPlayerGuidByName(playerName);
    Unit* target = ObjectAccessor::GetUnit(fromPlayer, playerGuid);

    if (!target || !target->IsPlayer()) {
        TellMaster("Can't find a player from name " + playerName);
        return;
    }
    ...
    for (std::list<uint32>::iterator it = itemIds.begin(); it != itemIds.end(); ++it)
    {
        ...
        if (pItem)
        {
            std::ostringstream itemReport;
            MakeItemText(pItem, itemReport, true);

            ItemPosCountVec dest;
            uint32 itemCount = pItem->GetCount();
            uint8 msg = targetPlayer->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, itemId, itemCount);

            if (msg != EQUIP_ERR_OK || dest.empty()) {
                errorReport << "Target player can't have " + itemReport.str() + ".";
                break;
            }

            m_bot->RemoveItem(pItem->GetBagSlot(), pItem->GetSlot(), true);
            targetPlayer->MoveItemToInventory(dest, pItem, true, true);
            targetPlayer->SendNewItem(pItem, itemCount, true, false);
            report << itemReport.str();
            itemCount++;
        }
    }
    ...
    if (errorReport.str().size() > 0)
        TellMaster(errorReport.str());
    else if (itemCount == 0)
        TellMaster("No items were given!");
}


// Logout player.
void PlayerbotAI::_HandleCommandLogout(Player& fromPlayer)
{
    ObjectGuid guid = m_bot->GetObjectGuid();
    CharacterDatabase.DirectPExecute("UPDATE characters SET online = 0 WHERE guid = '%u'", guid.GetCounter());
    m_mgr.LogoutPlayerBot(guid);
    TellMaster("I logout.");
}