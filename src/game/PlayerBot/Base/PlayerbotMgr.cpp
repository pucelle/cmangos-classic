...
// Set bot explored objective succeed.
void PlayerbotMgr::SetQuestExplored(uint32 questId, bool succeed)
{
    for (auto it = GetPlayerBotsBegin(); it != GetPlayerBotsEnd(); ++it)
    {
        Player* const bot = it->second;
        if (succeed)
            bot->AreaExploredOrEventHappens(questId);
        else
            bot->FailQuest(questId);
    }
}
